...
export const OAuthButton = ({
  ...
  const handleSignIn = async () => {
    setLoading(true);
    try {
      const { error } = await currentProvider.action();
      if (error) {
      ...
      onError?.(error.message || `Failed to sign in with ${currentProvider.name}`);
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      {loading ? "Signing in..." : `Continue with ${currentProvider.name}`}
    </button>
  );
};
